// FILE: /components/DayEndClosingModal.tsx
'use client';
...
const DayEndClosingModal: React.FC<DayEndClosingModalProps> = ({ isOpen, onClose, onSuccess, closingDate }) => {
    ...
    if (isOpen) {
      ...
      fetch(`/api/reports/daily-summary?date=${closingDate}`)
        ...
        .then(data => {
          ...
          setExpectedTotals(data.data.expectedTotals);
        })
        .catch(e => {
          setError(e.message);
  ...
  const handleSubmit = async () => {
    ...
    try {
        const payload = {
            closingDate,
            expectedTotals, // Include expected totals for a complete report
            actualTotals: {
                cash: calculatedActualCash, // Add calculated cash total
                card: parseFloat(actualCardTotal) || 0,
                upi: parseFloat(actualUpiTotal) || 0,
            },
            discrepancies: { // Add calculated discrepancies
          ...
          <div className="space-y-6">
                    ...
                    {denominations.map(d => (
                        <div key={d.value}>
                            ...
                            <input
                                ...
                                min="0"
                                placeholder='Count'
                                className="mt-1 w-full px-2 py-1.5 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-4 border rounded-lg">
                    ...
                    <label className="block text-sm font-medium text-gray-700">Actual Card Total (from POS slip)</label>
                ...
                <div className="p-4 border rounded-lg">
                    ...
                    <label className="block text-sm font-medium text-gray-700">Actual UPI Total (from app)</label>
